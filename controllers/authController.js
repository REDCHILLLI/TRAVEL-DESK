...
export const loginUser = async(req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ userId: user._id, email: user.email, role: 'user' }, process.env.JWT_SECRET, { expiresIn: '15d' });

        // <-- Replace the next line with the cookie version -->
        /*
        res.status(200).json({
          id: user._id,
        ...
        */
        ...
        res
            .cookie("access_token", token, {
                ...
                sameSite: "lax", // allow sending cookie to frontend
            })
            .status(200)
            .json({
                id: user._id,
